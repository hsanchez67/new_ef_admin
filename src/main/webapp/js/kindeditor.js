...
* // fix the IE bug: substitute relative url to absolute url
*******************************************************************************/
...
KE.browser = (function() {
	...
	return {
		...
		IE: (ua.indexOf('msie') > -1 && ua.indexOf('opera') == -1) || ua.indexOf('trident') > -1,
...
KE.setting = {
	...
	items : [
		...
		'flash', 'media', 'advtable', 'hr', 'emoticons', 'link', 'unlink', '|', 'about','inserttag'
	],
	colorTable : [
...
KE.selection = function(doc) {
	...
	// this function is immediately run. It sets some variables and probably isn't called outside.
	this.init = function() {
		...
		if (KE.browser.IE && !range.startContainer) {
			var rangeFunc = range.duplicate ? range.duplicate : range.cloneRange;
			if (range.item) {
				this.isControl = true;
				var el = range.item(0);
				...
				var getStartEnd = function(isStart) {
					var pointRange = rangeFunc.call(range); //range.duplicate();
					pointRange.collapse(isStart);
					var parentNode = pointRange.parentElement ? pointRange.parentElement() : (pointRange.startContainer ? pointRange.startContainer.parentNode : null);
					...
					var startPos = 0;
					var isEnd = false;
					var testRange = rangeFunc.call(range); //range.duplicate();
...
KE.format = {
	...
	getHtml : function(html, htmlTags, urlType) {
		...
		var htmlTagHash = {};
		//var fontSizeHash = ['9px', '10px', '12px', '14px', '16px', '18px', '24px'];
		var fontSizeHash = ['10px', '13px', '16px', '18px', '20px', '24px', '32px'];
		if (isFilter) {
...
KE.getComputedStyle = function(el, key) {
	var doc = el.ownerDocument,
		...
		val = style[jsKey] || style.getPropertyValue(key) || el.style[jsKey] || el.currentStyle[jsKey];
	 if (el.currentStyle) {
...
KE.util = {
	...
	createRange : function(doc) {
		...
		/* var func = doc.body.createTextRange;
		if (!func) {
			func = doc.createTextRange;
			if (!func) {
				console.log("createRange: createRange C");
				func = doc.createRange;
				return func.call(doc);
			}
			console.log("createRange: createTextRange B");
			return func.call(doc);
		} else {
			console.log("createRange: createTextRange A");
			return func.call(doc.body);
		} */
	},
	...
	getFullHtml : function(id) {
		...
		html += '</head>';
		html += '<body  data-module="helpPopup"  class="ke-content"></body>';
		html += '</html>';
		return html;
	},
	...
	getData : function(id, wyswygMode) {
		...
		// fix the IE bug: substitute relative url to absolute url
		// for example, href="[BusinessFacebook]" was replace to href="https:\\www.demandforced3.com/businessportal/campaigns/promotion/step/[BusinessFacebook]"
		var value = this.toData(id, g.iframeDoc.body.innerHTML);
		
		if (KE.browser.IE) {
			value = value.replace(/https:\/\/(\w|\.|\/)+\[/g, "[");
		}
	...
	setSelection : function(id) {
		...
		if (!KE.browser.IE || keSel.range.item 
			|| (keSel.range.parentElement && keSel.range.parentElement().ownerDocument === g.iframeDoc)
			 ) {
			g.keSel = keSel;
	...
	insertHtml : function(id, html) {
		...
		if (KE.browser.IE) {
			// alert("it's ie");
			this.select(id);
			// alert("g.range.text" + g.range.text);
			// alert(g.range.item);
			if (g.range.item) {				
				try {
				...
				g.range.pasteHTML(g.range.text + '<span id="__ke_temp_tag__">\u200B</span>' + html);
				var node = KE.$('__ke_temp_tag__', g.iframeDoc);
				node.parentNode.removeChild(node);
			}
...
//			var _x = $(obj[0]).offset().left;
//			var _y = $(obj[0]).offset().top;
//			var pos = {x: _x, y: _y};
...
KE.dialog = function(arg){
	...
	this.show = function() {
		...
		var Next = KE.$$('span');
		Next.className = 'ke-dialog-Next';
		Next.alt = KE.lang['Next'];
		Next.title = KE.lang['Next'];
...
KE.create = function(id, mode) {
	...
	iframe.setAttribute("frameBorder", "0");
	iframe.setAttribute("id", "customButtonIFrame");
...
//	editImage : 'Image properties',
	...
	imgExceedMaxWidth : 'You are attempting to embed an image that is wider than the maximum allowed width of *px. Use the percentage dropdown to reduce its size.',
...
KE.plugin.fontsize = {
	click : function(id) {
		...
		var size = 10;
		try {
			size = KE.queryCommandValue(KE.g[id].iframeDoc, cmd);
		} catch(e) {
	...
	exec : function(id, value) {
		...
		KE.util.execOnchangeHandler(id);
		//KE.util.execCommand(id, 'fontSize', value);
		this.menu.hide();
		KE.util.focus(id);	
	}
};
KE.customButtonArray = new Array();
KE.plugin.inserttag = {
	tags : {},
	click : function(id) {
		var cmd = 'inserttag';
		...
		for (value in this.tags) {
			//alert("test: " + value + ":" + this.tags[value]);
	...
	exec : function(id, value) {
		KE.util.insertHtml(id, this.tags[value]);
		KE.util.execOnchangeHandler(id);
		this.menu.hide();
		KE.util.focus(id);
	}
...
KE.plugin.textcolor = {
	click : function(id) {
		...
		var color = 0;
		try {
			color = KE.queryCommandValue(KE.g[id].iframeDoc, 'textcolor');
		} catch (e) {
	...
	exec : function(id, value) {
		...
		KE.util.execOnchangeHandler(id);
		KE.util.execCommand(id, 'textcolor', value);
		this.menu.hide();
		KE.util.focus(id);
	}
};
...
KE.plugin.link = {
	...
	click : function(id) {
		...
		dialog.show();
		$(".ke-mask").unbind("click").click(function() {
			dialog.hide();
		});
		$(".ke-dialog-yes input").addClass("button_action");
		$(".ke-dialog-no input").addClass("button_default");
	},
	exec : function(id) {
		var g = KE.g[id];
		KE.util.select(id);
...
//Insert custom button tag
function showColorPick(action){
	var actionTitle;
	if(action=="edit") 
		actionTitle="Edit Your Custom Button";
	else {
		actionTitle="Add Your Custom Button";
		$(".color_content span").css("border-color","#fff");
	}
	$(".editFormbox .title_s1").text(actionTitle);
	$("#mask1, .editFormbox").show();
	$(".editFormbox input[type=text]").val("");
	$("#color_hex_value").val("#008000");
	$("#current_color,#color_hex").css("background","#008000");
	
	$(".editFormbox").css("top",getScrollTop()+120);
		//add style to color pick
	$(".color_content span").each(function(){		
		$(this).click(function(){	  
			$(this).css("border-color","#369").siblings().css("border-color","#fff");
			var getColor = $(this).css("backgroundColor");
            var getColorHex =(getColor.toLowerCase().indexOf('rgb') <0)?getColor:rgb2hex(getColor);//get Hex value
            getColorHex=getColorHex.toUpperCase();
			$("#btnColor_hid").val(getColorHex);
			$("#btnColor_hid,#color_hex_value").val(getColorHex);
			$("#color_hex").css("background",getColorHex);			
		});
		
	});
}

$("#editBtnStyleC").live('click', function(){
            $("#mask1, .editFormbox").hide();
            $(".editFormbox input[type=text]").val("");
});
...
//End custom button tag insertion
})(KindEditor);
